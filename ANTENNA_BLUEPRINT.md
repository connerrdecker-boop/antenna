...
| Build state | **A1 shipped** · **A2 build half shipped + ratified**; calibration wiring shipped + ratified (golden set still pending — blocked on keys/egress, not on code) · **A3 build half shipped + ratified** (real providers await keys only) · **durability + erasure shipped + ratified** (Part X.2) · A4: /metrics pulled forward, export + measured run remain |
