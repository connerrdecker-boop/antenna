...
| **Scorer returns unparseable JSON on one profile** (watch-list, A2-national) | One profile doing this is the retry policy working as designed — Part 6.2 retries once, then flags `score_failed` for manual review, and the row is not lost. **A SECOND profile doing it is a parser or prompt defect, not bad luck, and gets fixed rather than retried.** On the record: `@hunterstein_wk` failed JSON validation on three attempts out of four across the v2 re-score and the golden `--rescore`, then succeeded on the fourth through the same path. Its bio is dense with bold-unicode maths characters and five distinct quoted DM keywords — the shape most likely to confuse string escaping. Do not widen the retry loop to paper over a second instance: a retry loop that hides a systematic escaping bug spends real money to produce the same failure more slowly |
